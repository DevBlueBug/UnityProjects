	...
	SubShader {
	    ...
	    pass{ 
			...
			fixed4 frag(vertex  i) : COLOR  {
				...
				/**
				float2 uv    = (i.uv ) * (_Frequency);
				float2 uvInt = float2(int(uv.x),int(uv.y));

				float2 gradient00 = getGradient(rand(uvInt.x)  ) ;
				float2 ratio00 = uv - uvInt;
				float2 ratio01 = ratio00 - float2(1,1);
				//float2 ratio = ratio02;
				float2 ratio = float2(smooth(ratio00.x),smooth(ratio00.y)); // * ratio02;


				float2 colorX =   getColor(uv, uvInt);
				float2 colorDown =	getColor(uv, uvInt);
				float2 colorUp =	getColor(uv, uvInt + float2(0,1) );
				
				float color00 = lerp(colorDown.x, colorDown.y, ratio.x );
				float color01 = lerp(colorUp.x, colorUp.y, ratio.x);
				float color = lerp(color00,color01, ratio.y) ;

				//float color = float4(dot(colorX,gradient00 ),0,0,1 );
				//float color = 1.0f;


				//return float4(lerp(colorX.x,colorX.y, ratio.x ),0,0,1 );
				//return float4( colorDown.x ,0,0,1);
				return float4( color ,color,color,1);
				return float4(colorDown.x,0,0,1);
				return float4(ratio.x,0,0,1);
				//return float4( lerp(colorX.x,colorX.y,ratio.x ) ,0,0,1);
				//return float4( ratio.x,ratio.y,0,1);
				//return float4( color * ratio.x,0,0,1);
				return float4( color,color,color,1);
				**/
			}
